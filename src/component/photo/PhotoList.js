                    ...
                    <div className="col-md-2 photoElement" key={idx}>
                        <PhotoElement photoData={photo} />
                        {
                            this.state.selectIndex.includes(photo.id) ?
                                <button type="button" className="btn btn-block btn-warning" onClick={() => this.handleClick(photo.id)}>취소하기</button> :
                                <button type="button" className="btn btn-block btn-primary" onClick={() => this.handleClick(photo.id)}>선택하기</button>
                        }
            ...
            <form>
                <div className="row photoBlock">
                    <div className="col-md-12">
                        <button type="button" className="btn btn-block btn-danger">파일 선택 삭제</button>
                    </div>
                    {photoElements}
                </div>
            </form>
